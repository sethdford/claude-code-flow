...
export class MigrationValidator {
  ...
  private async validateCommandFileContent(filePath: string, command: string, result: ValidationResult): Promise<void> {
    try {
      ...
      result.errors.push(`Failed to validate ${command}: ${(error as Error).message}`);
    }
  }
  ...
  private async validateConfiguration(projectPath: string, result: ValidationResult): Promise<void> {
    ...
    if (await fs.pathExists(roomodesPath)) {
      try {
        ...
        result.errors.push(`Invalid .roomodes file: ${(error as Error).message}`);
        check.passed = false;
      }
    }
    ...
    if (await fs.pathExists(claudePath)) {
      ...
      for (const file of files) {
        try {
          ...
          result.errors.push(`Cannot read file ${file}: ${(error as Error).message}`);
          check.passed = false;
        }
      }
    }
